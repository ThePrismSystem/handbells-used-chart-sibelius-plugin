...
RemoveChart(score) {
    ...
    deferred = RelocateSystemText(score, found.sections, found.sections + 1, False);
...
// Placement is NOT done here. Each copy is returned with the offsets it should
// end up with, for the caller to apply through ApplyDeferredText once the score
// has its final shape. The values travel in a sparse array because they have to
// cross a method boundary: an earlier attempt passed them in dynamic variables,
// and those are indirection over ordinary variables, which are method-local.
RelocateSystemText(score, fromBars, targetBarNum, musicalOnly) {
    system = score.SystemStaff;
    target = system.NthBar(targetBarNum);
    deferred = CreateSparseArray();
    ...
    while (b <= fromBars) {
        ...
        while (i < counter) {
            ...
            if (IsObject(copy)) {
                ...
                deferred.Push(CreateDictionary(
                    'copy', copy, 'dx', item.Dx, 'dy', item.Dy));
            }
            i = i + 1;
        }
...
// that system is the one about to disappear. Setting Dy while the chart bars
// are still there records it against a bar sitting low on the page, under the
// chart systems; once those bars go the bar rises to the top and the same Dy
// points off the page above it. That is what threw the title and subtitle off
// the score. Resetting to the style default avoided that but discarded the
// score's own positioning, which is what shifted the title block on every
// removal. Neither offset was wrong - the timing was.
ApplyDeferredText(deferred) {
    for i = 0 to deferred.Length {
        entry = deferred[i];
        copy = entry.copy;
        if (IsObject(copy)) {
            copy.Dx = entry.dx;
